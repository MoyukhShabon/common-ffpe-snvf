...
process_samples <- function(ffpe_tumor_annot, ff_tumor_annot, case_id_col, sample_name_col, model_name, ffpe_snvf_dir, outdir_root, vcf_ext = ".vcf") {
	...
	for (index in seq_len(nrow(ffpe_tumor_annot))){
		...
		vafsnvf_path <- file.path(ffpe_snvf_dir, model_name, sample_name, sprintf("%s.%s.snv", sample_name, model_name))
...
vafsnvf_all_score_truth <- combine_snv_score_truth(ffpe_tumoral_annot = ffpe_tumoral, sample_name_col = "sample_alias", model_name, score_truth_outdir)
