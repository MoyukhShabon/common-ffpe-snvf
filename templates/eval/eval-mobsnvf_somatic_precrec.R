...
process_samples <- function(ffpe_tumor_annot, ff_tumor_annot, case_id_col, variant_caller_col, sample_name_col, model_name, ffpe_snvf_dir, outdir_root, vcf_ext = ".vcf.gz") {
	...
	for (index in seq_len(nrow(ffpe_tumor_annot))){
		...
		mobsnvf_path <- file.path(ffpe_snvf_dir, tolower(wtype), model_name, sample_name, sprintf("%s.%s.snv", sample_name, model_name))
...
mobsnvf_all_score_truth <- combine_snv_score_truth(ffpe_tumoral_annot = ffpe_tumoral, sample_name_col = "vcf_fid", variant_caller_col = "workflow_type", model_name, score_truth_outdir)


# Stratify each variant caller
# mutect2
message("Performing Evaluation across mutect2 samples")
mobsnvf_mutect2_score_truth <- mobsnvf_all_score_truth[grepl("mutect2", tolower(mobsnvf_all_score_truth$workflow_type)), ]
mobsnvf_mutect2_overall_res <- evaluate_filter(mobsnvf_mutect2_score_truth, model_name, "all-mutect2-samples")
write_overall_eval(mobsnvf_mutect2_score_truth, mobsnvf_mutect2_overall_res, score_truth_outdir, eval_outdir, "all-mutect2-samples", model_name)

# varscan2
message("Performing Evaluation across varscan2 samples")
mobsnvf_varscan2_score_truth <- mobsnvf_all_score_truth[grepl("varscan2", tolower(mobsnvf_all_score_truth$workflow_type)), ]
mobsnvf_varscan2_overall_res <- evaluate_filter(mobsnvf_varscan2_score_truth, model_name, "all-varscan2-samples")
write_overall_eval(mobsnvf_varscan2_score_truth, mobsnvf_varscan2_overall_res, score_truth_outdir, eval_outdir, "all-varscan2-samples", model_name)

# muse
message("Performing Evaluation across muse samples")
mobsnvf_muse_score_truth <- mobsnvf_all_score_truth[grepl("muse", tolower(mobsnvf_all_score_truth$workflow_type)), ]
mobsnvf_muse_overall_res <- evaluate_filter(mobsnvf_muse_score_truth, model_name, "all-muse-samples")
write_overall_eval(mobsnvf_muse_score_truth, mobsnvf_muse_overall_res, score_truth_outdir, eval_outdir, "all-muse-samples", model_name)

# somaticsniper 
message("Performing Evaluation across somaticsniper samples")
mobsnvf_somaticsniper_score_truth <- mobsnvf_all_score_truth[grepl("somaticsniper", tolower(mobsnvf_all_score_truth$workflow_type)), ]
mobsnvf_somaticsniper_overall_res <- evaluate_filter(mobsnvf_somaticsniper_score_truth, model_name, "all-somaticsniper-samples")
write_overall_eval(mobsnvf_somaticsniper_score_truth, mobsnvf_somaticsniper_overall_res, score_truth_outdir, eval_outdir, "all-somaticsniper-samples", model_name)
