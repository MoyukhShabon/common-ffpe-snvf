...
process_samples <- function(ffpe_tumor_annot, ff_tumor_annot, case_id_col, variant_caller_col, sample_name_col, model_name, ffpe_snvf_dir, outdir_root, vcf_ext = ".vcf.gz", ct_only=FALSE) {
	...
	for (index in seq_len(nrow(ffpe_tumor_annot))){
		...
		microsec_path <- file.path(ffpe_snvf_dir, tolower(wtype), model_name, "outputs", sample_name, sprintf("%s.%s.tsv", sample_name, model_name))
		
		if(!file.exists(microsec_path)){
			message(sprintf("		Result table for %s does not exist at: %s", model_name, microsec_path))
			message(sprintf("		Skippping %s ...", sample_name))
			next
		}

		d <- read.delim(microsec_path)
		truth <- snv_union(matched_ff_paths)
		d <- preprocess_microsec(d, truth, ct_only)
		...
		microsec_res <- evaluate_filter(d, model_name, sample_name)

		# write results
		write_sample_eval(d, microsec_res, outdir_root, sample_name, model_name)
	}
}
...
microsec_all_score_truth <- combine_snv_score_truth(ffpe_tumoral_annot = ffpe_tumoral, sample_name_col = "vcf_fid", variant_caller_col = "workflow_type", model_name, score_truth_outdir)


# Stratify each variant caller
# mutect2
message("Performing Evaluation across mutect2 samples")
microsec_mutect2_score_truth <- microsec_all_score_truth[grepl("mutect2", tolower(microsec_all_score_truth$workflow_type)), ]
microsec_mutect2_overall_res <- evaluate_filter(microsec_mutect2_score_truth, model_name, "all-mutect2-samples")
write_overall_eval(microsec_mutect2_score_truth, microsec_mutect2_overall_res, score_truth_outdir, eval_outdir, "all-mutect2-samples", model_name)

# varscan2
message("Performing Evaluation across varscan2 samples")
microsec_varscan2_score_truth <- microsec_all_score_truth[grepl("varscan2", tolower(microsec_all_score_truth$workflow_type)), ]
microsec_varscan2_overall_res <- evaluate_filter(microsec_varscan2_score_truth, model_name, "all-varscan2-samples")
write_overall_eval(microsec_varscan2_score_truth, microsec_varscan2_overall_res, score_truth_outdir, eval_outdir, "all-varscan2-samples", model_name)

# muse
message("Performing Evaluation across muse samples")
microsec_muse_score_truth <- microsec_all_score_truth[grepl("muse", tolower(microsec_all_score_truth$workflow_type)), ]
microsec_muse_overall_res <- evaluate_filter(microsec_muse_score_truth, model_name, "all-muse-samples")
write_overall_eval(microsec_muse_score_truth, microsec_muse_overall_res, score_truth_outdir, eval_outdir, "all-muse-samples", model_name)

# somaticsniper 
message("Performing Evaluation across somaticsniper samples")
microsec_somaticsniper_score_truth <- microsec_all_score_truth[grepl("somaticsniper", tolower(microsec_all_score_truth$workflow_type)), ]
microsec_somaticsniper_overall_res <- evaluate_filter(microsec_somaticsniper_score_truth, model_name, "all-somaticsniper-samples")
write_overall_eval(microsec_somaticsniper_score_truth, microsec_somaticsniper_overall_res, score_truth_outdir, eval_outdir, "all-somaticsniper-samples", model_name)
